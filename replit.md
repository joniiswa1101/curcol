...
- **Real-Time Collaboration Canvas**: Shared whiteboard feature for visual collaboration. Database tables: `canvas_boards` (boards linked optionally to conversations) and `canvas_elements` (drawings, shapes, text, sticky notes). API routes: `GET/POST/PATCH/DELETE /api/canvas/boards` and `/api/canvas/boards/:id/elements`. WebSocket events: `canvas_join`, `canvas_leave`, `canvas_draw`, `canvas_cursor`, `canvas_clear` for real-time sync. Web page at `/canvas` with full drawing tools (freehand, rectangle, ellipse, line, arrow, text, sticky notes), color picker, undo/redo, zoom/pan, export PNG, and remote cursor display. Mobile tab "Canvas" with same drawing capabilities via HTML5 Canvas (web mode). Schema file: `lib/db/src/schema/canvas.ts`.
