...
- **Message Edit**: Mobile app supports message editing via long-press message bubble → edit button → modal input. PATCH endpoint: `PATCH /conversations/:conversationId/messages/:messageId`. Only sender can edit. Broadcasts update via WebSocket. Shows "(edited)" indicator on edited messages.
