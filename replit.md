...
- **Smart Message Summarization (P16)**: AI-powered chat summarization using OpenAI (gpt-4o-mini) via Replit AI Integrations proxy. API routes in `artifacts/api-server/src/routes/summarize.ts`: `POST /api/summarize/conversation/:conversationId` (TL;DR of last N messages with structured output), `POST /api/summarize/digest` (daily/weekly digest across all user conversations). Chat UI: Sparkles (✨) button in chat header opens inline summary panel with configurable message counts (50/100/200). Digest page at `/digest` with daily/weekly toggle, accessible from sidebar "AI Digest" nav item. Env vars: `AI_INTEGRATIONS_OPENAI_BASE_URL`, `AI_INTEGRATIONS_OPENAI_API_KEY`. Frontend files: `Chat.tsx` (TL;DR panel), `Digest.tsx` (digest page).
- **Translation & Language Learning (P17)**: AI-powered message translation and language learning via OpenAI. API routes in `artifacts/api-server/src/routes/translate.ts`: `POST /api/translate/message` (translate text to target language, 15 supported languages), `POST /api/translate/detect` (auto-detect language), `POST /api/translate/breakdown` (word-by-word analysis with pronunciation, meaning, part of speech, and grammar notes), `POST /api/translate/lesson` (generate mini language lessons from chat messages), `GET /api/translate/languages` (list supported languages). Chat UI: Right-click context menu on any message shows "Terjemahkan ke:" with 8 quick language buttons (ID/EN/JA/KO/ZH/ES/FR/DE), "Analisis Kata" for word breakdown, and "Mini Lesson" for language learning. Results display inline below the message bubble with color-coded panels (violet=translation, emerald=word breakdown, amber=mini lesson).
