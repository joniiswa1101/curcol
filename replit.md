...
- **Message Edit/Delete**: Mobile app supports message editing via long-press message bubble → edit button → modal input. PATCH endpoint: `PATCH /conversations/:conversationId/messages/:messageId`. Soft delete via trash icon (DELETE endpoint). Only sender can edit/delete. Broadcasts updates via WebSocket. Shows "(edited)" indicator and "Pesan telah dihapus" for deleted messages.
