...
- **Message Edit/Delete/Pin/Typing**: Mobile app supports message editing via long-press message bubble → edit button → modal input. PATCH endpoint: `PATCH /conversations/:conversationId/messages/:messageId`. Soft delete via trash icon (DELETE endpoint). Pin toggle via pin icon (PATCH pin endpoint) shows amber highlight and "Pinned" badge. Only sender can edit/delete. Any member can pin/unpin. Typing indicators show "X sedang mengetik..." banner with auto-stop after 2s inactivity. Broadcasts updates via WebSocket. Shows "(edited)" indicator and "Pesan telah dihapus" for deleted messages.
